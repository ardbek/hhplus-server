...
public class QueueControllerTest {
    ...
    void issueToken_success() throws Exception {
        ...
        QueueTokenIssueResponse response = new QueueTokenIssueResponse(1L, "queue-token-123", 1,
            TokenStatus.WAITING.name(), LocalDateTime.now(), LocalDateTime.now().plusMinutes(10));

        given(queueTokenService.issueToken(1L)).willReturn(response);
